...
var getAllcibochain = async function(username,userOrg) {
	try {
		...
		const request = {
			targets: targets,
		    chaincodeId: 'cibochain-app',
		    txId: tx_id,
	        fcn: 'queryAllcibochain',
	        args: ['']
	    };
		...
		logger.error('Failed to get all cibochain with error: %s', error.toString());
	}
};

var addcibochain = async function(username,userOrg,cibochainData) {
	var error_message = null;
	try {
		var array = cibochainData.split("-");
		...
		if (isProposalGood) {
			...
			event_hubs.forEach((eh) => {
				logger.debug('invokeAddcibochainEventPromise - setting up event');
				let invokeEventPromise = new Promise((resolve, reject) => {
					...
					eh.registerTxEvent(tx_id_string, (tx, code, block_num) => {
						logger.info('The addcibochain invoke chaincode transaction has been committed on peer %s',eh.getPeerAddr());
			...
			for(let i in results) {
				...
				if(typeof event_hub_result === 'string') {
					...
					var rezultat = {
						message: "cibochain created successfully!"
					};
					return rezultat;
				} else {
					...
					var rezultat = {
						message: "cibochain failed to create!"
					};
					return rezultat;
				}
			}
		...
		logger.error('Failed to add cibochain with error: %s', error.toString());
	}
};


var deletecibochain = async function(username,userOrg,cibochainData) {
	var error_message = null;
	try {
		var array = cibochainData.split("-");
		console.log(array);
		...
		if (isProposalGood) {
			...
			let event_hubs = channel.getChannelEventHubsForOrg();
			event_hubs.forEach((eh) => {
				logger.debug('invokeDeletecibochainEventPromise - setting up event');
				console.log(eh);
				let invokeEventPromise = new Promise((resolve, reject) => {
					let regid = null;
					let event_timeout = setTimeout(() => {
					...
					regid = eh.registerChaincodeEvent('cibochain-app', 'deleteEvent',
		...
		logger.error('Failed to delete cibochain with error: %s', error.toString());
	}
};
...
var changeOwner = async function(username,userOrg,ownerData) {
	var error_message = null;
	try {
		console.log("changing owner of cibochain catch: ");
...
var changeQuantity = async function(username,userOrg,quantityData) {
	try {
		console.log("changing quantity of cibochain: ");
...
exports.getAllcibochain = getAllcibochain;
exports.addcibochain = addcibochain;
exports.deletecibochain = deletecibochain;
