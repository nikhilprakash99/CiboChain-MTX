...
var logger = log4js.getLogger('cibochainWebApp');
var fs = require('fs');
...
//jwt
app.set('secret', 'cryptocibochainsecret');
...
app.get('/login/:user', async function (req, res) {
	...
	promise.then(function (doc) {
		if (doc) {
			...
			if (passwordIsValid) {
				//generate token
				let token = jwt.sign({ username: doc.username, orgname: doc.orgname }, 'cryptocibochainsecret', { expiresIn: '3h' });
...
app.use((req, res, next) => {
	...
	jwt.verify(token, 'cryptocibochainsecret', (err, decoded) => {
...
app.get('/get_all_cibochain', async function (req, res) {
	console.log(req.decoded);
	let response = await helper.getAllcibochain(req.decoded.username, req.decoded.orgname);
	res.json(JSON.parse(response[0].toString()));
});


app.get('/add_cibochain/:cibochain', async function (req, res) {
	req.params.cibochain = req.params.cibochain + '-' + req.decoded.username;
	console.log(req.params.cibochain);
	let response = await helper.addcibochain(req.decoded.username, req.decoded.orgname, req.params.cibochain);
	res.json({message: response.message});
});


app.get('/delete_cibochain/:kjmk', async function (req, res) {
	console.log("\nZahtjev je primljen");
	let response = await helper.deletecibochain(req.decoded.username, req.decoded.orgname, req.params.kjmk);
...
app.get('/check_owners/:kData', async function (req, res) {
	...
	if (req.decoded.username == array[0] && req.decoded.username == array[1]) {
		...
		for(var i = 0; i < approvedArray.length; i++) {
			if(approvedArray[i].sender == array[0] && approvedArray[i].cibochainID == array[5]) {
		...
		for(var i = 0; i < requestedArray.length; i++) {
			if(requestedArray[i].sender == array[0] && requestedArray[i].cibochainID == array[5]) {
				return res.json({ message: "Request is already sent"});
			}
		}
		requestedArray.push({sender: array[0], cibochainID: array[5]});
		...
		//pretraziti niz
		for(var i = 0; i < approvedArray.length; i++) {
			if(approvedArray[i].sender == array[1] && approvedArray[i].cibochainID == array[4]) {
				approvedArray.splice(i,1);
				return res.json({ message: "allow"});
			}
		}
		...
		for(var i = 0; i < requestedArray.length; i++) {
			if(requestedArray[i].sender == array[1] && requestedArray[i].cibochainID == array[4]) {
				return res.json({ message: "Request is already sent"});
			}
		}
		requestedArray.push({sender: array[1], cibochainID: array[4]});
		console.log(requestedArray);
		...
		for(var i = 0; i < approvedArray.length; i++) {
			if(approvedArray[i].sender == req.decoded.username && approvedArray[i].cibochainID == array[4]) {
				approvedArray.splice(i,1);
				firstApproved = true;
			}
			if(approvedArray[i].sender == req.decoded.username && approvedArray[i].cibochainID == array[5]) {
				approvedArray.splice(i,1);
				secondApproved = true;
			}		
		}
		...
		for(var i = 0; i < requestedArray.length; i++) {
			if(requestedArray[i].sender == req.decoded.username && requestedArray[i].cibochainID == array[4]) {
				firstRequestSent = true;
			}
			if(requestedArray[i].sender == req.decoded.username && requestedArray[i].cibochainID == array[5]) {
		...
		if(firstRequestSent == true && secondRequestSent == true) {
			...
			requestedArray.push({sender: req.decoded.username, cibochainID: array[5]});
			...
			requestedArray.push({sender: req.decoded.username, cibochainID: array[4]});
